...
`config/public.json` contains the current installation's public URL and anon client key. These identify the public backend; they grant no staff authority. Keep this key until the matching reviewed backend configuration is deployed; the release handlers also support publishable keys. Never put a service-role, provider secret or signing key in a client configuration.
...
Start with [the deployment guide](docs/RELEASE.md). It covers the shared backend, Hostinger website, Android release workflow and iPhone/Watch TestFlight path. [Mobile setup](docs/MOBILE-RELEASE.md) lists the exact signing inputs and commands.
