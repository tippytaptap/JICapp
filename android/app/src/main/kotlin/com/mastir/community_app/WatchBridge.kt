    ...
    init {
        ...
        MethodChannel(messenger, "community/watch").setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "publishTimeline", "publishCounter" -> {
                    val payload = call.arguments as? String ?: ""
                    val timeline = call.method == "publishTimeline"
                    if (if (timeline) !WatchPayload.validTimeline(payload) else !WatchPayload.validCounter(payload)) {
                        ...
                        Wearable.getDataClient(app).putDataItem(request)
                            ...
                            .addOnFailureListener { result.error("watch_unavailable", "Connect your paired watch and try again.", null) }
                    }
                }
...
class PhoneWatchDataService : WearableListenerService() {
    override fun onDataChanged(events: DataEventBuffer) {
        for (event in events) {
            if (event.type != DataEvent.TYPE_CHANGED || event.dataItem.uri.path != WatchPayload.WATCH_COUNTER) continue
