...
const prayerNames = {'fajr': 'Fajr', 'zuhr': 'Dhuhr', 'asr': 'Asr', 'maghrib': 'Maghrib', 'isha': 'Isha'};
const capabilities = ['content', 'media', 'events', 'announcements', 'prayer_times', 'team', 'livestream', 'tv', 'broadcast', 'forms_contact', 'forms_madrassah', 'forms_itikaaf', 'users', 'audit', 'delete_content'];

bool can(Record? profile, String permission) => profile?['is_active'] == true &&
    (profile?['is_owner'] == true || (profile?['permissions'] as List? ?? []).contains(permission));
bool active(Record? profile) => profile?['is_active'] == true;
bool owner(Record? profile) => active(profile) && profile?['is_owner'] == true;

List<Record> records(dynamic data) => data is List ? data.whereType<Map>().map((r) => Record.from(r)).toList() : [];

bool safeWebUrl(String value) {
  final url = Uri.tryParse(value);
  return url != null && url.scheme == 'https' && url.host.isNotEmpty &&
      url.userInfo.isEmpty && !value.contains(RegExp(r'[\s\\]'));
}
...
bool adultProgramme(Record p) {
  ...
  return groups.contains('education') && !groups.contains('youth') && !groups.contains('madrassah');
}
...
List<Record> weeklySessions(List<Record> programmes) => [
  for (final p in programmes.where(adultProgramme))
    for (final s in records(p['sessions']))
      if (s['day'] is int && s['day'] >= 1 && s['day'] <= 7 &&
          (RegExp(r'^(?:[01]\d|2[0-3]):[0-5]\d$').hasMatch('${s['time']}') || s['relativeTo'] == 'maghrib'))
        {...s, 'title': s['label'] ?? p['title'], 'programmeId': p['id']}
]..sort((a, b) => (a['day'] as int).compareTo(b['day'] as int));

tz.TZDateTime centreNow(String zone, [DateTime? now]) => tz.TZDateTime.from(now ?? DateTime.now(), tz.getLocation(zone));
...
tz.TZDateTime? prayerMoment(String date, dynamic value, String zone) {
  ...
  var hour = int.parse(match[1]!); final minute = int.parse(match[2]!);
  if (minute > 59 || hour > 23 || (match[3] != null && (hour < 1 || hour > 12))) return null;
  if (match[3] != null) hour = hour % 12 + (match[3]!.toUpperCase() == 'PM' ? 12 : 0);
  return tz.TZDateTime(tz.getLocation(zone), day.year, day.month, day.day, hour, minute);
}
...
List<Record> prayerTimeline(List<Record> days, String zone) => [
  for (final day in days)
    for (final p in prayerNames.entries)
      if (prayerMoment('${day['d_date']}', day['${p.key}_begins'], zone) case final moment?)
...
double qiblaBearing(double latitude, double longitude) {
  ...
  return (math.atan2(math.sin(delta), math.cos(lat) * math.tan(kaaba) - math.sin(lat) * math.cos(delta)) * 180 / math.pi + 360) % 360;
}
...
String formsCsv(List<Record> rows) => [
  ...
  for (final row in rows) [row['id'], row['kind'], row['status'], row['created_at'], row['payload']].map(csvCell).join(',')
].join('\r\n');
