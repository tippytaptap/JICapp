...
class Organisation {
  ...
  static Future<Organisation> load() async => Organisation(
    Record.from(
      jsonDecode(await rootBundle.loadString('assets/organisation.json')),
    ),
  );
  ...
  String? resolveImage(dynamic value) {
    ...
    if (value.startsWith('/') &&
        !value.startsWith('//') &&
        !value.contains('\\')) {
      return Uri.parse(website).resolve(value).toString();
    }
