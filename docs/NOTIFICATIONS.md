...
- The registered FCM token and its account owner are kept in secure device storage. Registration uses `register_push_device(p_token, p_platform)`, never a user-supplied recipient. Token refresh updates the registration. The server rejects rebinding a token from another account.
