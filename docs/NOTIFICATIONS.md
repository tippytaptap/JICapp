...
1. Register the Android application and iOS bundle in the organisation's Firebase project. Download Android `google-services.json` to `android/app/` and iOS `GoogleService-Info.plist` to `ios/Runner/`. The iOS build's `Copy Firebase configuration` phase copies the optional file after checking its bundle ID; do not add a duplicate Copy Bundle Resources entry. This service calls `Firebase.initializeApp()` using those native defaults. If choosing FlutterFire-generated `firebase_options.dart` instead, deliberately change that one initialisation call to pass `DefaultFirebaseOptions.currentPlatform`; do not initialise Firebase unconditionally in `main.dart`.
2. The Google services Gradle plugin version 4.5.0 is already declared and applied only when `android/app/google-services.json` exists. This conditional keeps an unconfigured preview build usable. Flutter's pinned Firebase plugins already provide their native dependencies; do not add Firebase Auth or a second account database.
