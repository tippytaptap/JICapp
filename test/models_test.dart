...
void main() {
  ...
  test('Disabled owners and cosmetic labels never grant access', () {
    ...
    expect(can({'is_active': true, 'staff_kinds': ['admin']}, 'users'), false);
    expect(can({'is_active': true, 'permissions': ['forms_contact']}, 'forms_contact'), true);
    expect(can({'is_active': true, 'permissions': ['forms_contact']}, 'forms_madrassah'), false);
  });
  test('Adult courses exclude pupil and youth programmes', () {
    expect(adultProgramme({'groups': ['education','madrassah']}), false);
    expect(adultProgramme({'audience': 'adult','groups': ['education']}), true);
    expect(weeklySessions([{'audience':'adult','title':'Lesson','sessions':[{'day':8,'time':'12:00'},{'day':1,'time':'28:00'},{'day':5,'relativeTo':'maghrib'}]}]).length, 1);
  });
  test('Prayer times use the centre timezone through BST', () {
    expect(prayerMoment('2026-07-01','13:00','Europe/London')!.toUtc().hour,12);
    expect(prayerMoment('2026-12-01','13:00','Europe/London')!.toUtc().hour,13);
  ...
  test('Birmingham Qibla points southeast', () => expect(qiblaBearing(52.4862,-1.8904), closeTo(119, 3)));
